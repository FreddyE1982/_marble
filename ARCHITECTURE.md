...
- `Brain`: n-dimensional space that can be either:
  ...
  Snapshot serialization requires the `pigz` compressor for multi-threaded gzip output. When `pigz` is missing the runtime clones the upstream repository, builds the binary, installs it under `~/.local/bin`, and removes the clone before retrying; a failed build raises an error rather than silently downgrading. Compression level remains configurable via `snapshot.compress_level` (default `2`), clamped to the 1–9 gzip range for compatibility.
