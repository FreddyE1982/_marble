...
class AdvancedSelfAttentionPluginSuiteTests(unittest.TestCase):
    def make_brain(self):
        ...
        if len(idxs) > 1:
            b.add_neuron(idxs[1], tensor=[0.0], connect_to=idxs[0])
        return b
    ...
    def test_plugins_register_learnables(self):
        ...
        plugins = {
            "noise_profiler": ["noise_variance", "spatial_factor"],
            "entropy_router": ["entropy_threshold", "high_temp", "low_temp"],
            "adaptive_grad_clip": [],
            "conv1d_inserter": [],
            "findbestneurontype": [],
        }
