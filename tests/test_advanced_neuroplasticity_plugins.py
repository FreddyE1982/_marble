...
class AdvancedNeuroplasticityPluginSuiteTests(unittest.TestCase):
    def make_brain(self):
    ...
    def test_plugins_register_learnables(self):
        ...
        plugins = {
            "synapse_scaler": ["scale"],
            "random_pruner": ["prune_threshold"],
            "bias_shift": ["shift_lr"],
            "connection_rewire": ["rewire_prob"],
            "spectral_normalizer": ["norm_rate"],
        }
        for name, params in plugins.items():
            with self.subTest(name=name):
                ...
                print("neuroplasticity plugin", name, "learnables", list(learnables.keys()))
                for p in params:
